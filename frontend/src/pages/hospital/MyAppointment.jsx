...
const MyAppointments = () => {
  ...
  const [appointments, setAppointments] = useState(
    location.state?.freshAppointments || []
  );
  ...
  const handleJoinMeeting = async (appointmentId) => {
    ...
    try {
      ...
      toast.error('Room not active. Please join at your scheduled time.', { id });
    }
  };
  ...
  const handlePayment = async (appointmentId) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Payment Verified', { id });
        fetchAppointments();
      }
    } catch {
      toast.error('Transaction declined', { id });
    }
  };

  const handleCancel = async (appointmentId) => {
    try {
      const { data } = await API.patch(`/appointments/cancel/${appointmentId}`);
      if (data.success) {
  ...
  /* ── Get doctor image — handles both old and new data structures ── */
  const getDoctorImage = (item) => {
    return item.doctorId?.image           // ✅ new: image directly on Doctor model
      || item.doctorId?.userId?.image     // fallback: old data structure
      || '';
  };

  const getDoctorName = (item) => {
    return item.doctorId?.name            // ✅ new: name directly on Doctor model
      || item.doctorId?.userId?.name      // fallback: old data structure
      || 'Medical Specialist';
  };
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <AnimatePresence mode="popLayout">
            {appointments.length > 0 ? appointments.map(item => {
              const docImage = getDoctorImage(item);
              ...
              return (
                <motion.div layout key={item._id}
                  ...
                  style={{
                    background:  '#fff',
                    border:      `1px solid ${item.status === 'Cancelled' ? '#f8fafc' : '#f1f5f9'}`,
                    boxShadow:   item.status === 'Cancelled' ? 'none' : '0 2px 16px rgba(0,0,0,0.04)',
                    opacity:     item.status === 'Cancelled' ? 0.5 : 1,
                    filter:      item.status === 'Cancelled' ? 'grayscale(1)' : 'none',
                  ...
                  <div style={{ position:'relative', flexShrink:0 }}>
                    {docImage ? (
                      ...
                      <div style={{
                        ...
                        <IoPersonOutline size={32} color="#93c5fd"/>
                      </div>
                    )}
                  ...
                  <div className="appt-info">
                    ...
                    <p style={{ fontSize:10, fontWeight:900, color:'#2563eb', textTransform:'uppercase', letterSpacing:'0.15em', marginBottom:14 }}>
                      {item.doctorId?.specialization || 'General Practice'}
                    </p>
                  ...
                  <div className="appt-actions" style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    ...
                    {!item.payment && item.status !== 'Cancelled' ? (
                      ...
                      <div style={{
                        ...
                        padding:'clamp(12px,2vw,18px)', borderRadius:16, fontSize:10, fontWeight:900,
                    ...
                    {/* Status badge */}
                    <div style={{
                      padding:'8px 0', borderRadius:10, fontSize:9, fontWeight:900,
                      textTransform:'uppercase', textAlign:'center', borderBottom:'2px solid',
                      ...(item.status === 'Cancelled'  ? { color:'#cbd5e1', borderColor:'#f1f5f9' }
                        ...
                        : item.status === 'Completed'  ? { color:'#16a34a', borderColor:'#bbf7d0', background:'#f0fdf4' }
