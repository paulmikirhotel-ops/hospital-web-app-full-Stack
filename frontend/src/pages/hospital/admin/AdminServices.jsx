...
import {
  IoAddOutline, IoTrashOutline, IoCloseOutline, IoCreateOutline
...
const AdminServices = () => {
  ...
  const [formData, setFormData]   = useState({ title: '', description: '', isAvailable: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editId) {
        res = await API.put(`/services/update/${editId}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        if (!file) return toast.error('Please upload an icon');
        res = await API.post('/services/add', data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Permanently delete this department?')) {
      try {
        ...
        if (data.success) { toast.success('Service removed'); fetchServices(); }
      } catch (err) {
  ...
  const handleToggleStatus = async (id) => {
    try {
      const { data } = await API.patch(`/services/toggle-status/${id}`);
      if (data.success) {
  ...
  const openEditModal = (service) => {
    ...
    setFormData({ title: service.title, description: service.description, isAvailable: service.isAvailable });
    setShowModal(true);
  };
    ...
    <div className="p-4 md:p-8 bg-white rounded-[3rem] shadow-sm border border-slate-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service) => (
          <div key={service._id} className="bg-slate-50/50 rounded-[2.5rem] p-8 border border-transparent hover:border-blue-100 hover:bg-white transition-all relative">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => openEditModal(service)} className="p-2 bg-white rounded-lg text-slate-400 hover:text-blue-600 border border-slate-100 shadow-sm">
                  <IoCreateOutline size={16} />
                </button>
                <button onClick={() => handleDelete(service._id)} className="p-2 bg-white rounded-lg text-slate-400 hover:text-red-600 border border-slate-100 shadow-sm">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-xl rounded-[3.5rem] p-10 shadow-2xl relative">
            <button onClick={closeModal} className="absolute top-8 right-8 text-slate-400 hover:text-slate-900">
              <IoCloseOutline size={30} />
            </button>
            <h3 className="text-3xl font-black text-slate-900 mb-2 tracking-tight">
              {editId ? 'Edit Dept' : 'New Dept'}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <label htmlFor="fileInput" className="w-full flex flex-col items-center justify-center py-8 border-2 border-dashed border-slate-200 rounded-[2rem] cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all">
                  {file
                    ? <span className="text-xs font-black text-emerald-600 uppercase">{file.name}</span>
                    : <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Upload Department Icon</span>
                  }
                </label>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-2 p-5 bg-slate-50 rounded-2xl border-none font-bold text-slate-700 focus:ring-2 focus:ring-blue-100 outline-none"
                />
              </div>
