      ...
      {show.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', height:'100%', justifyContent:'flex-end' }}>
            <motion.div
              initial={{ height:0 }} animate={{ height:`${Math.max(pct,2)}%` }}
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {app.payment && (
          <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest hidden sm:block">✓ Paid</span>
        )}
      </div>
    </motion.div>
  );
...
const AdminDashboard = () => {
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 bg-[#FBFDFF] min-h-screen">
      ...
      {/* FILTER BAR */}
      <FilterBar filters={filters} setFilters={setFilters} onApply={handleApplyFilter} onClear={handleClearFilter} loading={filterLoading}/>
